...
#	#Extract stats of how much each clump is round or rectangular
#	r.object.geometry -m input=clumped_$gfile output=objgeom.csv separator=comma
#	#Insert here magic program to separate treecrown that are touching
#	#color tree crown by area
#	r.colors -e map=clump_vclean_r_area_$gfile color=roygbiv
#	r.area --overwrite input=clumped_vclean_$gfile output=clump_vclean_r_area_$gfile lesser=2
#	#output to disk the area of crown
#	r.out.gdal input=clump_vclean_r_area_$gfile output=area_$gfile
#	#output to disk clump by clump
