...
# Purpose:  Separate circles that merge borders with math morpho operators
...
void usage()
{
	...
	printf( "-----------------------------------------\n");
	printf( "./separate_crowns in > out.csv\n");
	printf( "-----------------------------------------\n");
...
//				private(val,row,col,count,j,k,m,n,lon,lat,err,ar_col,ar_row)
*/
	for(val=min;val<max;val++){
		/*clean arrays (sequential run only, to remove if parallel)*/
		for(j=0;j<ARRAY_SIZE;j++){
			ar_col[j]=0.0;
			ar_row[j]=0.0;
		}	
		...
		//More than 200 border pixels is a group of olive tree to separate
		if (count>=200 && count<2000){
			...
			if( subin != NULL ) free( subin );
			if( subout != NULL ) free( subout );
		}
	}/* END OF CLUMP LOOP */
	/*Free Memory*/
	...
	ff2d(map,nX);
	/*write to output file not working with OpenMP !*/
	//fclose(fp);	
	GDALClose(hD);
	return(EXIT_SUCCESS);
}
